...
SimulatedRobot::SimulatedRobot(dart::dynamics::SkeletonPtr skeleton)
    : skeleton(skeleton), qp(skeleton->getNumDofs(), 0, 0) {
...
VectorX SimulatedRobot::getJointRequest(const State& state) {
    std::cout << "RETRIEVING JOINT REQUEST" << std::endl;

    // Define QP Problem
    const int d = skeleton->getNumDofs();
    ...
    pos_error = state.desired_left_foot.pose.getVector() - state.left_foot.pose.getVector();  // 6 x 1
    std::cout << "Left Foot Pos Error: " << pos_error.transpose().format(Config::CleanFmt) << std::endl;
    vel_error = state.desired_left_foot.getVelocity() - state.left_foot.getVelocity();        // 3 x 1
    ...
    pos_error = state.desired_right_foot.pose.getVector() - state.right_foot.pose.getVector();  // 6 x 1
    std::cout << "Right Foot Pos Error: " << pos_error.transpose().format(Config::CleanFmt) << std::endl;
    vel_error = state.desired_right_foot.getVelocity() - state.right_foot.getVelocity();        // 3 x 1
    ...
    pos_error = state.desired_torso.pose.rotation.getRPY() - state.desired_torso.pose.rotation.getRPY();  // 3 x 1
    ...
    qp.update(H, g, nullopt, nullopt, nullopt, nullopt, nullopt);
    qp.solve();
    VectorX command = qp.results.x.segment(6, d - 6);
