...
void FootTrajectoryGenerator::update(State& state) {
    EndEffector swing_foot
        = plan.footsteps.front().support_foot == Foot::right ? state.left_foot : state.right_foot;
    ...
    if (plan.support_phase == SupportPhase::DOUBLE) {
        ...
        swing_foot.pose
            = Pose3(RotationMatrix::aroundZ(desired_theta), Vector3(desired_pos(0), desired_pos(1), 0));
    ...
    if (plan.support_phase == SupportPhase::DOUBLE) {
        state.left_foot_contact = true;
        state.right_foot_contact = true;
    } else {
        state.left_foot_contact = plan.footsteps.front().support_foot == Foot::left;
        state.right_foot_contact = plan.footsteps.front().support_foot == Foot::right;
    }
}
